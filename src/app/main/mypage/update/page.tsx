  ...
  return (
    <div className="animate-fade-in flex flex-col pb-20">
      ...
      <Card className="w-full border-0 shadow-none">
        <CardContent>
          <div className="flex flex-col">
            ...
            <div className="flex flex-row items-center gap-5">
              ...
              <Input className="w-[50%] text-sm bg-gray-100 border-muted" name="name" value={profileInfo?.name} readOnly />
            </div>
            <Separator className="my-4 border-t-2 border-muted-foreground" />
            <div className="flex flex-row items-center gap-5">
              ...
              <Input
                ...
                onChange={(e) => {
                  setProfileInfo((prev) => (prev ? { ...prev, nickName: e.target.value } : prev));
                }}
                readOnly={true}
              />
              <Button
            ...
            <div className="flex flex-row items-center gap-5">
              ...
              <Input className="w-[50%] text-sm bg-gray-100 border-muted" name="email" value={profileInfo?.email || "이메일 미설정"} readOnly />
            </div>
            <Separator className="my-4 border-t-2 border-muted-foreground" />
            <div className="flex flex-row items-center gap-5">
              <span className="font-bold text-base">주소</span>
              <Input className="w-[50%] text-sm" name="address" value={profileInfo?.address || "주소 미설정"} readOnly />
              <Button
                onClick={() => {
            ...
            <div className="flex flex-row items-center gap-5">
              ...
              <Input className="w-[50%] text-sm" name="comment" value={profileInfo?.comment || ""} readOnly onChange={(e) => setProfileInfo((prev) => (prev ? { ...prev, comment: e.target.value } : null))} placeholder="자기소개가 없습니다" />
              <Button
                onClick={(e) => {
            ...
            <div className="flex flex-row items-center gap-5">
              ...
              <Input className="w-[50%] text-sm bg-gray-100 border-muted" name="comment" value={profileInfo?.joinDate.split("T")[0]} readOnly />
            </div>
            <Separator className="my-4 border-t-2 border-muted-foreground" />
            {profileInfo?.platform === 0 && (
              <>
                <div className="flex flex-row items-center gap-5">
                  ...
                  <Button onClick={updatePassword} className="w-[30%] bg-red-600 text-white hover:bg-red-700">
                    비밀번호 변경
                  </Button>
                </div>
                <Separator className="my-4 border-t-2 border-muted-foreground" />
        ...
        <CardFooter className="px-6 pt-0 flex justify-center"></CardFooter>
      </Card>
