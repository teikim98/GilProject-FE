      ...
      <AlertDialog open={isPopupOpen} onOpenChange={setIsPopupOpen}>
        <AlertDialogContent className="max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid w-full items-center gap-4">
            ...
            {imageUrl ? <img src={profileImg} alt="Profile" className="w-12 h-12 rounded-full object-cover" /> : <Camera className="w-12 h-12 p-2 bg-muted rounded-full" />}
          </div>

          {/* 이미지 Url 넣는 Input */}
          <div className="relative">
            <div className="flex flex-col space-y-1.5">
              <Input name="imageUrl" type="text" value={inputImg?.name} placeholder="여기를 클릭하여 수정할 이미지를 업로드해주세요" />
            </div>
            <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={handleImageUpload} id="profile-image-upload" />
          </div>
